...
def my_histogram(channel_0, channel_1, channel_2, nbins, bins_range):
    # Compute the histogram of each channel separately
    histogram_0 = np.histogram(channel_0, bins=nbins, range=bins_range)
    histogram_1 = np.histogram(channel_1, bins=nbins, range=bins_range)
    histogram_2 = np.histogram(channel_1, bins=nbins, range=bins_range)

    # Concatenate the histograms
    features = np.concatenate((hist_0[0], hist_1[0], hist_2[0])).astype(np.float64)

    # Return normalized the result
...
def compute_color_histograms(cloud, using_hsv=True):
    ...
    return my_histogram(ch_0, ch_1, ch_2, nbins=32, bins_range=(0, 256))
...
def compute_normal_histograms(normal_cloud):
    ...
    return my_histogram(norm_x, norm_y, norm_z, nbins=32, bins_range=(-1, 1))